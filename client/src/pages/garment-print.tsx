...
export default function GarmentPrintPage() {
  ...
  const handleThermalPrint = async () => {
    ...
    if (!snapshot?.connected || !snapshot.selectedPrinter.trim()) {
      const message = "Couldn't find your printer. Check it's connected properly and try again.";
      setPrinterDialogMode("missing-printer");
    ...
    try {
      ...
      setPrinterIssueMessage("");
      setIsReconnectDialogOpen(false);
      toast({ title: "Label sent", description: `${garment.code} sent to ${snapshot.selectedPrinter}` });
    } catch (error) {
      const description = describeQzError(error);
        ...
        <div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Preview updates in real-time with size, offsets, QR, and header.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <PrintSettingLabel help="TSPL is recommended for this printer. Use ZPL only if your hardware supports it and you've validated it.">Thermal language</PrintSettingLabel>
              ...
              <div className="space-y-2">
                <PrintSettingLabel help="Quick presets for common sizes. Custom keeps your manual values.">Label preset</PrintSettingLabel>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2"><PrintSettingLabel help="Shifts all content horizontally in dots. Use only for fine printer adjustments.">Offset X (dots)</PrintSettingLabel><Input type="number" value={settings.offsetX} onChange={(e) => setSettings((prev) => ({ ...prev, offsetX: Number(e.target.value) || 0 }))} /></div>
              ...
              <div className="space-y-2"><PrintSettingLabel help="QR can contain only the code or the full garment URL.">QR content</PrintSettingLabel><Select value={qrMode} onValueChange={(value) => setQrMode(value as QrPayloadMode)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="code">Code only</SelectItem><SelectItem value="url">Garment URL</SelectItem></SelectContent></Select></div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <Button onClick={handleThermalPrint} disabled={!garment?.code || isPrintingThermal} className="flex-1">
                  <Printer className="mr-2 h-4 w-4" />
                  {isPrintingThermal ? "Sending..." : "Print thermal"}
                </Button>
                <Button variant="outline" onClick={handleBrowserPrint} className="flex-1">Print browser</Button>
              </div>
              <Button type="button" variant="ghost" onClick={handleOpenPrinterHelp} className="w-full">I'm having print issues</Button>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>What you see here uses the same layout logic as the thermal engine.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-md border p-3"><p className="text-sm font-medium">Code</p><p className="mt-1 font-mono text-sm text-muted-foreground">{garment?.code || garmentCode || "-"}</p></div>
                <div className="rounded-md border p-3"><p className="text-sm font-medium">QR Payload</p><p className="mt-1 break-all text-sm text-muted-foreground">{qrValue || "-"}</p></div>
              </div>
              {isLoading && <p className="text-sm text-muted-foreground">Loading garment…</p>}
            </CardContent>
          </Card>
        </div>
      </div>
